...
const PostActions = () => {
  ...
  const DeletePhotos = async (post_media: string[]) => {
    const filenames = post_media.map((url) => url.split('data/')[1]);
    const { data, error } = await Supabase.storage.from('data').remove(filenames);

    if (error) throw error;
  };

  const DeleteItem = async (postId: number, post_media: string[]) => {
    await Promise.all([DeletePost(postId), post_media.length > 1 && DeletePhotos(post_media)]);
    const newPosts = posts.filter((post) => post.id !== postId);
    setPosts(newPosts);
  };

  const EditPost = async (post: PostClass) => {
    const { data, error } = await Supabase.from('posts').update(post).match({ id: post.id }).select().single();

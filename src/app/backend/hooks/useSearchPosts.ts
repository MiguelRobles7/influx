...
const SearchPosts = ({ u, s, so, c, t, rs, re, tg, o, ow, setPosts }: Props) => {
    ...
    const searchPosts = async () => {
        ...
        if (so === "ascending") SupabaseQuery = SupabaseQuery.order("posted_at", { ascending: true });
            ...
            SupabaseQuery = SupabaseQuery.order("posted_at", { ascending: false });
        }
        ...
        if (u || ow) {
            newData = newData.filter((post) => {
                const postUser = users?.find((user: UserClass) => user.uuid === post.author.uuid);
        ...
        newData.forEach((post) => {
            ...
            post.origin =
                communities?.find((community: CommunityClass) => community.uuid === post.origin.uuid) ||
                new CommunityClass();
        });
        ...
        if (s) {
            if (["upvotes", "popularity"].includes(s)) {
                ...
                newData.sort((a, b) => {
                    return (
                        ((b.price || 0) + (b.range_start || 0) - ((a.price || 0) + (a.range_start || 0))) *
                        (so && so === "ascending" ? -1 : 1)
                    );
                });
            }

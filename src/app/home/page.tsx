...
export default function Register() {
  ...
  useEffect(() => {
    if (user.uuid) setTimeout(() => router.push('/'), 1000);
  }, [user.uuid]);
  ...
  return (
    <main className="flex flex-col w-screen h-screen bg-[url('/images/bg-home.jpg')] bg-cover bg-center bg-no-repeat">
      ...
      <div className="flex flex-row bg-cover bg-center bg-no-repeat bg-[url('/backgrounds/landing.jpg')] rounded-l-lg h-full aspect-square justify-center items-center gap-10">
        <h6
          className="text-white font-black text-[8rem] leading-[5rem] tracking-tighter italic w-[40rem]"
        ...
        <div className="w-[18rem] flex flex-col gap-8">
          <h6
            className="text-white font-bold text-[2rem] leading-[1.6rem] italic tracking-tight"
            style={{ WebkitTextStroke: '1px', WebkitTextStrokeColor: 'black' }}
          >
          ...
          <Link
            href="/auth/register"
            className="bg-white text-black py-3 px-7 flex items-center w-fit rounded-full cursor-pointer hover:bg-slate-900 hover:text-violet-300 transition-colors duration-200 border-[1px] border-black"
          >
